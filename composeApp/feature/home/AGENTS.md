...
| 依存 | 種別 | 用途 |
|---|---|---|
| `projects.composeApp.ui` | `api` | `AppRoute`/`AppRouter`、`SystemScaffold` 等の共通コンポーネント、`XxxPagingSource` |
...
- 本モジュールは `composeResources/` を持たないため `compose-components-resources` への依存は不要（文言を追加する場合は `composeResources/` を持つ他の `feature/<name>` モジュールの依存構成を参照）。
    ...
    .flatMapLatest { keyword ->
        Pager(PagingConfig(XxxPagingSource.PAGE_SIZE)) {
            XxxPagingSource(
                ...
                fetcher = { key -> fetchItems(keyword, key) }
            )
        }.flow
    }.cachedIn(viewModelScope)
```

- `XxxPagingSource` は `ui` の共通コンポーネントとして提供される（→ decisions.md D-05）。Screen 側は `collectAsLazyPagingItems()` で受け取る。
...
- ナビゲーション引数（`AppRoute.Xxx.NavArgs.keyword`）を検索欄の初期値に使う場合、`LaunchedEffect(keyword)` で `viewModel.updateSearchKeyword`/`initSearchKeyword` を呼び分ける（deep link 経由か通常遷移かの分岐）。
